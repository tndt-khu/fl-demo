...
def check_valid_folder_structure(p):
    ...
    for folder in FL_EXAMPLES:
        if not os.path.isfile(os.path.join("backend", folder, "README.md")) and not os.path.isfile(os.path.join(
                "backend", folder, "generate_configs.py")):
            p.error(
...
def setup_parser():
    ...
    p.add_argument("--dataset", "-d",
                   help="Dataset code from backend", type=str, required=True)
...
if __name__ == '__main__':
    ...
    generate_party_config(config_model, num_parties, conn_type,
                          dataset, party_data_path, folder_configs, l_ipaddress
                          None)
    generate_party_config(config_model, num_parties, conn_type,
                          dataset, party_data_path, folder_configs
                          None)
